...
The creation of a card starts with the post [Debit Card POST](card.en#debit-card-post), [Credit Card POST](card.en#credit-card-post) or [Prepaid Card POST](card.en#prepaid-card-post) call, depending on the acquired issuance service.
...
>Physical cards are created in a default status of `PendingAck`for security during the logistics of delivery. See [Ack Reception](card_ops.en#ack-reception-post) for details on physical card activation.